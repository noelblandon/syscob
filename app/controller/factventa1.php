...
if(isset($_GET['recibo'],$_GET['nombrec'],$_GET['prod'],$_GET['vcant'],$_GET['precio'],$_GET['vdesc1'],$_GET['total'],$_GET['cont'])){
  	session_start();
...
$this->SetXY(198, 44.5);
  ...
  function fun(){
    $pixel = 84;
    $total_pago = 0;
    for ($i = 0; $i <= $_GET['cont']-1 ; $i++){
      $vcant = explode(",", $_GET["vcant"]);
      $prod = explode(",", $_GET["prod"]);
      $precio = explode(",", $_GET["precio"]);
    /* datos del body*/
    $this->SetFont('Courier','', 10);
    $this->SetXY(11, $pixel);
    $this->MultiCell(20,5,utf8_decode($vcant[$i]),0,'C');
    $this->SetXY(31, $pixel);
    $this->MultiCell(65,5,utf8_decode($prod[$i] ),0,'L');
    $this->SetXY(96, 96);
    $this->MultiCell(20,5,utf8_decode(),0,'C');
    $this->SetXY(116, $pixel);
    $this->MultiCell(20,5,utf8_decode('$ '.$precio[$i] ),0,'C');
    /* datos del body*/
    $this->SetFont('Courier','', 10);
